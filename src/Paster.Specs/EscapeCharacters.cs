...
namespace Paster.Specs
{
    ...
    public class EscapeCharacters
    {
        ...
        public void PastingDoubleQuotesInScenario(EnvironmentClipboard clipboard, GherkinPaster sut, TestEnvironment environment)
        {
            ...
            "and a scenario containing a 'double quote'"
                .And(() => clipboard = FakesLibrary.CreateShim("Scenario: With a \" character"));

            "When the single line is pasted"
                .Then(() => sut.PasteGherkin(clipboard));

            "Then the 'double quote' character has been removed"
                .Then(() => environment.LinesWritten.Any(l => l.Contains("\""))
                                       .Should()
                                       .BeFalse());
        }
        ...
        public void PastingDoubleQuotesInInstructions(EnvironmentClipboard clipboard, GherkinPaster sut, TestEnvironment environment)
        {
            ...
            "and a line containing a 'double quote'"
                .And(() => clipboard = FakesLibrary.CreateShim("Given a \" character"));

            "When the line is pasted"
                .Then(() => sut.PasteGherkin(clipboard));
